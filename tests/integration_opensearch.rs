//! Integration tests: OpenSearch via ElasticsearchAdapter (same wire protocol).
//!
...
#[tokio::test]
async fn opensearch_query_returns_results() {
    test_infra::containers::docker_check();
    let inst = test_infra::containers::opensearch_container().await;
    ...
    .expect("ElasticsearchAdapter::new for OpenSearch");
...
#[tokio::test]
async fn opensearch_health_check() {
    test_infra::containers::docker_check();
    let inst = test_infra::containers::opensearch_container().await;
    let adapter = ElasticsearchAdapter::new(ElasticsearchConfig {
        base_url: inst.base_url.clone(),
    ...
    assert!(healthy, "OpenSearch should report healthy");
}
